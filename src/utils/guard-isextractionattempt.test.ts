...
describe("isExtractionAttempt", () => {
  ...
  describe("should not flag legitimate single-library queries", () => {
    ...
    // Registry-name exemption: these all contain a bulk-extraction keyword
    // ("list", "all") inside a real library name and were being refused.
    it.each([
      "flash-list",
      "@shopify/flash-list",
    ...
    ])("allows registry entry whose name contains a guard keyword: %s", (query) => {
      expect(isExtractionAttempt(query)).toBe(false);
    });
  });
