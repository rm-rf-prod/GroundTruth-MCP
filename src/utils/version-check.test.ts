...
describe("isNewerVersion", () => {
  it("detects major version bump", () => {
    expect(isNewerVersion("3.0.2", "1.0.0")).toBe(true);
  });
  ...
  it("handles v prefix", () => {
    expect(isNewerVersion("v3.0.2", "v2.0.0")).toBe(true);
  });
});

describe("formatUpdateNotice", () => {
  it("includes the new version", () => {
    const notice = formatUpdateNotice("3.0.2");
    expect(notice).toContain("3.0.2");
