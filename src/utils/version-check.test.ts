...
describe("isNewerVersion", () => {
  it("detects major version bump", () => {
    expect(isNewerVersion("3.0.0", "2.4.0")).toBe(true);
  });

  it("detects minor version bump", () => {
    expect(isNewerVersion("2.5.0", "2.4.0")).toBe(true);
  });
  ...
  it("returns false when current is newer", () => {
    expect(isNewerVersion("2.3.0", "2.4.0")).toBe(false);
  });

  it("handles v prefix", () => {
    expect(isNewerVersion("v3.0.0", "v2.4.0")).toBe(true);
  });
});
