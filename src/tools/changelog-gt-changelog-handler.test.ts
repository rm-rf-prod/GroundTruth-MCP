...
describe("gt_changelog handler", () => {
  ...
  describe("cache hit", () => {
    it("returns cached result without fetching", async () => {
      // Cache stores a {text, structuredContent} envelope so a hit returns the
      // same shape as a miss.
      vi.mocked(docCache.get).mockReturnValue(
        JSON.stringify({
          text: "CACHED_RESPONSE",
          structuredContent: { libraryId: "vercel/next.js", displayName: "Next.js", qualityScore: 0.9 },
        }),
      );
      const result = await handler({ libraryId: "vercel/next.js" });
      expect(result.content[0]!.text).toBe("CACHED_RESPONSE");
      expect(result.structuredContent).toMatchObject({ displayName: "Next.js", cached: true });
      expect(fetchGitHubReleases).not.toHaveBeenCalled();
    });
  });
