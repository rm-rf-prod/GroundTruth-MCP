...
function formatResults(matches: LibraryMatch[]): string {
  ...
  const lines: string[] = [
    ...
    "",
    "Use the ID from one of these results with ws_get_docs.",
    "",
  ];
...
Call this FIRST before ws_get_docs unless you already know the library ID.
...
- ws_resolve_library({ libraryName: "fastapi", query: "authentication" })`,
