...
describe("gt_best_practices handler", () => {
  ...
  describe("response format", () => {
    ...
    it("includes effectiveTopic in topic header when version provided without topic", async () => {
      ...
      vi.mocked(lookupById).mockReturnValue(entry);
      vi.mocked(fetchViaJina).mockResolvedValue(BP_CONTENT);
      const result = await handler({ libraryId: "vercel/next.js", version: "15.2.0" });
    ...
    it("combines topic and version into effectiveTopic", async () => {
      ...
      vi.mocked(lookupById).mockReturnValue(entry);
      vi.mocked(fetchViaJina).mockResolvedValue(BP_CONTENT);
      const result = await handler({ libraryId: "vercel/next.js", topic: "routing", version: "15.2.0" });
      expect(result.content[0]!.text).toContain("routing v15.2.0");
    });
