...
describe("ws_auto_scan handler", () => {
  ...
  describe("no dependency files found", () => {
    ...
    it("suggests ws_get_docs in no-manifests message", async () => {
      await mockNoFiles();
      const result = await handler({});
      expect(result.content[0]!.text).toContain("ws_get_docs");
    });
  ...
  describe("requirements.txt scanning", () => {
    it("detects Python packages from requirements.txt", async () => {
      await mockRequirementsTxt("flask==2.0.0\nrequests>=2.28.0\n# comment\n");
      vi.mocked(fetchDocs).mockResolvedValue(makeFetchResult());
      const result = await handler({});
  ...
  describe("response format", () => {
    ...
    it("mentions unmatched packages via ws_resolve_library when present", async () => {
      await mockPackageJson({ "unknown-xyz-private-pkg": "^1.0.0" });
      vi.mocked(fetchDocs).mockResolvedValue(makeFetchResult());
      const result = await handler({});
      expect(result.content[0]!.text).toContain("ws_resolve_library");
    });
  });
});
