...
describe("DiskCache", () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), "ws-mcp-diskcache-test-"));
  });
  ...
  async function makeDiskCache(dir: string) {
    ...
    // Use DiskCache via dynamic module with patched WS_CACHE_DIR
    process.env.WS_CACHE_DIR = dir;
    vi.resetModules();
  ...
  it("persists data across separate cache instances (different requires)", async () => {
    ...
    vi.resetModules();
    process.env.WS_CACHE_DIR = tmpDir;
  ...
  afterEach(() => {
    delete process.env.WS_CACHE_DIR;
  });
});
